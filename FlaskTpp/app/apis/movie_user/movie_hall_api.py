...
class UserMovieHallResource(Resource):
    ...
    def get(self,id):
        ...
        #订单筛选，排挡筛选，订单状态，锁单的
        movie_orders_buyed=MovieOrder.query.filter(MovieOrder.o_hall_movie_id==id).filter(or_(MovieOrder.o_status==ORDER_STATUS_PAYED_NOT_GET,
                                                       MovieOrder.o_status==ORDER_STATUS_GET)).all()
        movie_order_lock=MovieOrder.query.filter(MovieOrder.o_hall_movie_id==id).filter(MovieOrder.o_status==ORDER_STATUS_NOT_PAY).filter(\
            MovieOrder.o_time > datetime.now()).all()
        seats=[]
