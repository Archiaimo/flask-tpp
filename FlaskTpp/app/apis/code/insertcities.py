...
def insert_cities(cities_json):
    ...
    keys=cities.keys()
    db=pymysql.Connect(host='localhost',port=3306,user='root',password='Archi920328!',
                       database='flasktpp')
    cursor=db.cursor()
    for key in keys:
        #print(key)
        ...
        for city in cities_letter:
            ...
            cursor.execute("INSERT INTO city(letter_id,c_id,c_parent_id,c_region_name,c_city_code,c_pinyin) VALUES (%d,"
                           "%d,%d,'%s',%d,'%s');" % (letter_id,c_id,c_parent_id,c_region_name,c_city_code,c_pinyin))
